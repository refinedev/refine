// Thanks to Vercel/Next.js -> https://github.com/vercel/next.js/blob/canary/release.js

// section -> label
const conventionalNamingMap = {
    Features: "feat",
    "Bug Fixes": "fix",
...
const getSectionForPullRequest = (pullRequest) => {
    ...
    for (const [section, label] of Object.entries(conventionalNamingMap)) {
...
const groupByConventionalNaming = async (commits, github) => {
    // Initialize the sections object with empty arrays
    const sections = Object.keys(conventionalNamingMap).reduce(
        (sections, section) => {
            sections[section] = [];
            return sections;
        },
...
function cleanupPRTitle(title) {
    const regexp = new RegExp(
        `^(${Object.values(conventionalNamingMap).join(
            "|",
        )})(\\s)?((\\(|\\[).*(\\)|\\]))?:`,
        "i",
    );
...
module.exports = async (markdown, metadata) => {
    ...
    const sections = await groupByConventionalNaming(commits.all, github);
