...
`useCheckError` calls the `checkError` method from the [`authProvider`](api-references/providers/auth-provider) under the hood.
...
- The promise returned from `checkError` method of [`authProvider`](api-references/providers/auth-provider) can reject with a custom url instead.
