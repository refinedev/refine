...
`useCheckError` calls the `checkError` method from the [`authProvider`](/api-references/providers/auth-provider.md) under the hood.
...
We have a logic in [`authProvider`](/api-references/providers/auth-provider.md)'s `checkError` method like below.
...
- The promise returned from `checkError` method of [`authProvider`](/api-references/providers/auth-provider.md) can reject with a custom url instead.
