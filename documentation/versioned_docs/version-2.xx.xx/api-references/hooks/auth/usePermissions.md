...
`usePermissions` calls the `getPermissions` method from the [`authProvider`](/api-references/providers/auth-provider.md) under the hood.
...
- We have a logic in [`authProvider`](/api-references/providers/auth-provider.md)'s `getPermissions` method like below.
