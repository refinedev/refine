...
- It uses the `deleteOne` method as the **mutation function** from the [`dataProvider`](/core/docs/3.xx.xx/api-reference/core/providers/data-provider/) which is passed to `<Refine>`.
...
| errorNotification                                                                                   | Unsuccessful Mutation notification                                                                 | [`SuccessErrorNotification`](/core/docs/3.xx.xx/api-reference/core/interfaces#successerrornotification) | "Error (status code: `status`"      |
...
> `*`: These props have default values in `RefineContext` and can also be set on [`<Refine>`](/core/docs/3.xx.xx/api-reference/core/components/refine-config) component. `useDelete` will use what is passed to `<Refine>` as default but a local value will override it.
