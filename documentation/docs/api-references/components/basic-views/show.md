...
When clicked on, delete button executes the `useDelete` method provided by the `dataProvider` and the edit button redirect the user to the record edit page.
...
| actionButtons   | Passes to the `extra` property of the `<Card>`   | `React.ReactNode`                                                 | `<SaveButton>` and depending on your `<Resource>` configuration (`canDelete` prop) |
| isLoading       | Passes to the `loading` property of the `<Card>` | `boolean`                                                         | `false`                                                                            |
