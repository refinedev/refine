...
**Refine** handles realtime data operations through [Live Provider](/docs/core/providers/live-provider/index) which provides a common interface for any integration. Once integrated, you'll get realtime updates across your app out of the box, without needing a further configuration.
...
See the [Integrated Hooks](/docs/core/providers/live-provider/index#integrated-hooks) section for more information.
...
**refine** will use this subscribe method in the [`useSubscription`](/docs/core/hooks/realtime/use-subscription/index) hook.
...
> For more information, refer to the [useSubscription documentation&#8594](/docs/core/hooks/realtime/use-subscription/index)
...
This `publish` is used in [realated hooks](#publish-events-from-hooks). When `publish` is used, subscribers to these events are notified. You can also publish your custom events using [`usePublish`](/docs/core/hooks/realtime/use-publish/index).
...
**refine** will provide this publish method via the [`usePublish`](/docs/core/hooks/realtime/use-publish/index) hook.
...
When you call the [`useList`](/docs/core/hooks/data/use-list/index), [`useOne`](/docs/core/hooks/data/use-one/index) or [`useMany`](/docs/core/hooks/data/use-many/index) hooks, they will call the `subscribe` method of the live provider.
...
**Refine** hooks will create a subscription query using the parameters of the [useSubscription](/docs/core/hooks/realtime/use-subscription/index) hook and listen to it. When a live event is received, it will call the `onLiveEvent` method of the `useSubscription` hook.
