...
[Check out TanStack Table's `useTable` reference page to learn more about the usage and see it in action.](/core/docs/packages/list-of-packages#installation)
...
- **mode**: Whether to use server side pagination or not.
  ...
  - When `cursor` is selected, cursor-based pagination will be used. Navigation is done via `cursor.goToNextPage()` and `cursor.goToPreviousPage()` returned by the hook. See [`useTable` cursor pagination docs](/core/docs/data/hooks/use-table#paginationcursor) for details.
