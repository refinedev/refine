...
`syncWithLocationLocalStorage` use `refine-table-sync-with-location-resource.name` key for the storage.
...
| syncWithLocationLocalStorage                                 | Sortings, filters, page index and records shown per page are tracked by locale storage. Use default `refine-table-sync-with-location-resource.name` key for the storage | `boolean`                                                                      | Value set in [Refine][refine swl]. If a custom resource is given, it will be `false` |
