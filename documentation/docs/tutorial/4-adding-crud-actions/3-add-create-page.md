...
In this post, we manually add the code for the `create` page of `blog_posts` resource, which is used to create new a blog post record.
...
Now, if we view the blog post `create` page in the browser at <a href="http://localhost:3000/blog-posts/create" rel="noopener noreferrer nofollow">localhost:3000/blog-posts/create</a>, we see the same forms being rendered, except the `Show the auto-generated code` is missing because we removed the `<MuiCreateInferencer />` component.
