...
-   `useDataGrid` hook, imported from `@refinedev/mui` package, has been developed by using the `useTable` hook imported from `@refinedev/core` package. So, It provides all the features of the `useTable` hook.
...
Now, we can see the list page in the browser at <a href="http://localhost:3000/blog_posts" rel="noopener noreferrer nofollow">localhost:3000/blog_posts</a>
