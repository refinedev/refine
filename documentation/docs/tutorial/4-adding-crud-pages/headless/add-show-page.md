...
-   `useNavigation` is a **refine** hook that is used to navigate between pages. In this case, we are using it to navigate to the `edit` and `list` pages when the user clicks on the "Edit" and "Products List" buttons.
...
3. Replace the `HeadlessInferencer` component with the `ProductShow` component.
