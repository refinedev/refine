...
![1-build-and-download](https://imgbox.com/0QHGZVUh)
...
## refine `<App />` Component

We carry out extensive inspections of the generated code throughout this tutorial, but let's first start with the `<App />` component. It looks like this:
...
We delve into these props starting [Unit 3](/docs/tutorial/understanding-refine-props/data-provider) with `dataProvider`. However, **refine** has a very powerful tool, [**the Inferencer**](https://refine.dev/docs/api-reference/mui/components/inferencer/), which automatically coded all the CRUD pages we have already in use in our admin panel app. We, therefore, spare the next post to preview the pages generated by **Inferencer**.

<Checklist>
<ChecklistItem id="getting-started-7">
