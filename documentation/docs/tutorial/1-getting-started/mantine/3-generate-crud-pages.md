...
You can Learn more about [how Inferencer works](/docs/packages/list-of-packages) and [how to use it with Mantine](/docs/ui-integrations/mantine/components/inferencer) in our documents.

## How to use Inferencer

The `@refinedev/inferencer` package provides the `<MantineInferencer/>` component, which can be imported from `@refinedev/inferencer/mantine`. It is used to generate CRUD pages based on your API response with Mantine components.
...
The `resources` prop will be explained in detail in [Unit 4](/docs/tutorial/understanding-resources). For now, you can assume that the resource is a collection of data on your API used in the app.
...
How **refine** apps communicate with the API via the `dataProvider` will be explained in [Unit 3](/docs/tutorial/understanding-dataprovider).
...
You will learn how to create CRUD pages manually using the code generated by Inferencer as a reference in [Unit 5](/docs/tutorial/adding-crud-pages/mantine).
