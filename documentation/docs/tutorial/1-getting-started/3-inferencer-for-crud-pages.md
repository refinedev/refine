...
If we look inside `src/pages` directory, we can see that pages related to `blog-posts` and `categories` are generated by us. When we investigate the pages, we see that there's no effort at coding the UI elements for these pages. For example, `list` page for `blog-posts` look like this:
...
If we examine closely, we can see that only a `<MuiListInferencer />` element is returned from the `<BlogListComponent />`. And that's it.

This element, an Inferencer tailored for Material UI components in this case, is generating the code for the pages we have in our app. We can see that all other pages are also generated by appropriate Inferencers.
