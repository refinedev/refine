...
First, you'll need to create an `OktaAuth` instance, then use the `createAuthProvider` method to create an auth provider. You can then pass the auth provider to the [`<Refine />`](/core/docs/core/refine-component) componentcore/docs/data/hooks/use-form.
