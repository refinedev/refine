    ...
    return (
        <TutorialSandpack
            ...
            finalFiles={{
                ...
                "auth-provider.ts": {
                    code: AuthProviderWithTokenExpiration,
                },
                "login.tsx": {
                    code: LoginComponentWithUseLogin,
                },
...
const AuthProviderWithTokenExpiration = /* tsx */ `
// TODO: change this
import { AuthProvider } from "@refinedev/core";
...
export const authProvider: AuthProvider = {
    ...
    // login method receives an object with all the values you've provided to the useLogin hook.
    login: async ({ email, password }) => {
        const response = await fetch("https://api.fake-rest.refine.dev/auth/login?expires=15", {
            method: "POST",
            body: JSON.stringify({ email, password }),
            headers: {
                "Content-Type": "application/json",
...
export const AddTokenExpirationToAuthProviderLoginMethod = () => {
    const { sandpack } = useSandpack();
    ...
    return (
        <TutorialUpdateFileButton
            onClick={() => {
                sandpack.updateFile(
                    "/auth-provider.ts",
                    AuthProviderWithTokenExpiration,
                );
                sandpack.setActiveFile("/auth-provider.ts");
            }}
        />
