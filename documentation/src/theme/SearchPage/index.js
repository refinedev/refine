...
function SearchPageContent() {
  ...
  const searchScope = new URLSearchParams(search).get("scope");
  const isBlogScope = searchScope === "blog";
  const activeIndexName = isBlogScope ? "refine_blog" : indexName;
  ...
  const algoliaHelper = algoliaSearchHelper(algoliaClient, activeIndexName, {
    hitsPerPage: 15,
    advancedSyntax: true,
    disjunctiveFacets: isBlogScope ? [] : ["language", "docusaurus_tag"],
  });
  algoliaHelper.on(
    "result",
    ({ results: { query, hits, page, nbHits, nbPages } }) => {
      ...
      const items = hits.map((hit) => {
        ...
        const titles = hierarchy
          ? Object.keys(hierarchy).map((key) =>
              sanitizeValue(hierarchy[key]?.value),
            )
          : [];

        const docsTitle = titles.length > 0 ? titles[titles.length - 1] : null;
        const title =
          docsTitle ??
          sanitizeValue(
            hit?._highlightResult?.headline?.value ??
              hit?.headline ??
        ...
        const rawUrl = hit?.url ?? hit?.url_without_anchor ?? "/";
        let normalizedUrl = rawUrl;

        try {
          normalizedUrl = processSearchResultUrl(rawUrl);
        } catch {
        ...
        return {
          title,
          url: normalizedUrl,
          summary: summaryValue ? `${sanitizeValue(summaryValue)}...` : "",
          breadcrumbs: docsTitle ? titles.slice(0, -1) : ["Blog"],
        };
      });
  ...
  const showVersionFilters =
    docsSearchVersionsHelpers.versioningEnabled && !isBlogScope;
  useEffect(() => {
    if (!loaderRef) {
      return undefined;
    }
        ...
        <form className="row" onSubmit={(e) => e.preventDefault()}>
          <div
            className={clsx("col", styles.searchQueryColumn, {
              "col--9": showVersionFilters,
              "col--12": !showVersionFilters,
            })}
          >
            <input
              type="search"
