...
[`useTable`](/docs/ui-integrations/ant-design/hooks/use-table/index) in the code above is a refine hook that fetches data from an API and wraps it with various helper hooks that make it compatible with Ant's `<Table>` component.
...
[`useForm`](/docs/ui-integrations/ant-design/hooks/use-form/index) is a refine hook for handling form data. It offers adapters that let refine integrate with Ant design's `<Form>` component.
