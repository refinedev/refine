...
Note that pnpm has excellent support for monrepo and offline mode. 
## pnpm CLI
Pnpm has a cool CLI. Here are some of the basic commands:

- Create a package.json file ```pnpm init```
- Download all the packages listed as dependencies in package.json ``` pnpm install```
- Download a particular version of a package and add to the list of dependencies in package.json ``` pnpm add [module_name]@[version]```
- Uninstall and remove a package from the list of dependencies in package.json ``` pnpm remove [module_name]```
- Print a tree of locally installed modules ```pnpm list```
- 
