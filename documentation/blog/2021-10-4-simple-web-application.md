...
function App() {
  return <Refine dataProvider={dataProvider(supabaseClient)} authProvider={authProvider} LoginPage={Login} />;
}
...
import { Row, Col, AntdLayout, Card, Typography, Form, Input, Button, Checkbox } from "@pankod/refine";
import "./styles.css";
...
```tsx
import { Create, Form, Input, IResourceComponentsProps, Upload, useForm, RcFile } from "@pankod/refine";
import { IMovies } from "interfaces";
            ...
            <Upload.Dragger
              ...
              customRequest={async ({ file, onError, onSuccess }) => {
                try {
                  ...
                  await supabaseClient.storage.from("refineflix").upload(`public/${rcFile.name}`, file, {
                    cacheControl: "3600",
                    upsert: true,
                  });

                  const { data } = supabaseClient.storage.from("refineflix").getPublicUrl(`public/${rcFile.name}`);
...
```tsx
import React from "react";
import { Edit, Form, Input, IResourceComponentsProps, RcFile, Upload, useForm } from "@pankod/refine";
  ...
  return (
    <Show
      ...
      pageHeaderProps={{
        ...
        extra: (
          <>
            <EditButton onClick={() => push(`/admin/movies/edit/${record?.id}`)} />
            <RefreshButton />
          </>
        ),
      }}
    >
      ...
      <Space wrap>
        {record?.images ? (
          record.images.map((img) => <ImageField key={img.name} value={img.url} title={img.name} width={200} />)
        ) : (
...
import { AdminMovieList, AdminMovieCreate, AdminMovieShow, AdminMovieEdit } from "./pages/admin/movies";
...
```tsx
import { IResourceComponentsProps, Card, Space, useList, useNavigation } from "@pankod/refine";
          ...
          <Card
            ...
            cover={
              movie.images?.length > 0 ? (
                ...
                <img alt="default" src="https://cdn.pixabay.com/photo/2019/04/24/21/55/cinema-4153289_960_720.jpg" />
              )
            }
            loading={isLoading}
...
```tsx
import { useShow, Show, Typography, IResourceComponentsProps, Space, ImageField } from "@pankod/refine";
import { Layout } from "components";
