...
This means mutation hooks such as [`useCreate()`](https://refine.dev/docs/api-reference/data/hooks/useCreate/) can now publish events to the `pixels` channel and consumer hooks like [`useList()`](http://localhost:3000/docs/api-reference/data/hooks/useList/) are able to get instant updates for any change to `pixels`.
