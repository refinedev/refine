...
const shadcnRegistry = [
  ...
  },
  {
    ...
    files: [
      { path: "ui/toast.tsx", type: "registry:ui" },
      { path: "hooks/use-toast.ts", type: "registry:hook" },
      { path: "ui/toaster.tsx", type: "registry:ui" },
    ],
  },
  {
    name: "toggle",
...
shadcnRegistry.forEach((item) => {
  parsedRegistry.push(item.name);
});

function getRegistryItemUrl(itemName, registryUrl) {
  const registryBaseUrl = registryUrl.replace(/\/registry\.json$/, "");
  return `${registryBaseUrl}/${itemName}.json`;
}

fs.writeFileSync("shadcn-list.txt", parsedRegistry.join("\n"));
