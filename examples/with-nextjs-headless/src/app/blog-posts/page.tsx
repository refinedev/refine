...
export default function BlogPostList() {
  ...
  const { tableQuery, current, pageSize, setPageSize, setCurrent } = useTable({
  ...
  const { data: categoryData, isLoading: categoryIsLoading } = useMany({
    resource: "categories",
    ids: records?.map((item: any) => item?.category?.id).filter(Boolean) ?? [],
    queryOptions: {
      enabled: !!records,
    },
  });
    ...
    <div>
                ...
                <td>
                  {categoryIsLoading
                    ...
                    : categoryData?.data?.find(
                        (item: any) => item.id === record.category?.id,
                      )?.title}
                </td>
                <td>{t(`posts.fields.status.${record.status}`)}</td>
                <td>
                  {record.createdAt &&
      ...
      <div>
        <span>
          ...
          >
            {[...Array(Math.ceil((data?.total ?? 0) / pageSize))].map(
              (_, i) => (
